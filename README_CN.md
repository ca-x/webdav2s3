...
| 数据库模式 | DATABASE_URL / DATABASE_PATH | 多后端，Web UI，用户管理 |
...
2. 如果是首次启动，系统会跳转到 `http://localhost:8080/admin/setup`
3. 在初始化向导中创建第一个管理员账户
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/setup/status` | 查询是否已完成初始化 |
| POST | `/api/v1/setup/init` | 创建首个管理员（仅首次） |
