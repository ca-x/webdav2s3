...
func (_q *UserQuery) QueryBackends() *S3BackendQuery {
	query := (&S3BackendClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithBackends(opts ...func(*S3BackendQuery)) *UserQuery {
	query := (&S3BackendClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBackends; query != nil {
		if err := _q.loadBackends(ctx, query, nodes,
			func(n *User) { n.Edges.Backends = []*S3Backend{} },
			func(n *User, e *S3Backend) { n.Edges.Backends = append(n.Edges.Backends, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadBackends(ctx context.Context, query *S3BackendQuery, nodes []*User, init func(*User), assign func(*User, *S3Backend)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.BackendsTable)
		s.Join(joinT).On(s.C(s3backend.FieldID), joinT.C(user.BackendsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.BackendsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.BackendsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*S3Backend](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "backends" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
